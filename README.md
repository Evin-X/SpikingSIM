...
In the following example, we first reconstruct intensity map (a) from raw spikes (c) and use it to simulate spikes (d). The image (b) is reconstructed from simulated spikes (d). The visualization shows that the synthetic data (d) generated by *SpikingSIM* are quite similar with the raw data (c).  
<div align=center>
